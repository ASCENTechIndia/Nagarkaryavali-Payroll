...
const FrmPayHeadMst = () => {
  ...
  useEffect(() => {
    if (mergeInOptions.length > 0 && pendingMergeId && formikRef.current) {
  ...
  const handleDelete = async () => {
    const result = await Swal.fire({
      title: "Are you sure?",
      text: "You want to delete this Pay Head?",
    ...
    try {
      ...
      const payload = {
        userId: userId,
        corpId: Number(ulbId),
        payHeadId: Number(values.payHeadId),
