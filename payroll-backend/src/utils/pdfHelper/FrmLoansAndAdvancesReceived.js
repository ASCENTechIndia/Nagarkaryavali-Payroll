...
const LoansAdvancesPDFHelper = async ({
  rows,
  payHeadsName,
  fromDate,
  ulbId,
  ...
  const templatePath = path.resolve(
    __dirname,
    "../../templates/FrmLoansAndAdvancesReceived.html"
  );
  ...
  // Format the header
  const header = `List Of ${payHeadsName || "Loans And Advances"}`;
  ...
  const reportRows = rows.map((row, index) => ({
    ...
    deductionAmt: Number(row.DEDUCTION_AMT || 0).toLocaleString("en-IN", {
  ...
  const fileName = `LoansAdvances_${Date.now()}.pdf`;
