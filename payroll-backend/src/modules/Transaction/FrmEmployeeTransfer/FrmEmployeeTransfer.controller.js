...
exports.saveEmployeeTransfer = async (req, res) => {
  try {
    console.log("✅ POST /save-transfer called");
    const result = await service.saveEmployeeTransferService(req.body);
    res.json(result);
  } catch (error) {
    console.error("❌ Error in saveEmployeeTransfer:", error);
    res.status(500).json({ 
      success: false, 
      message: error.message || "Failed to save transfer" 
    });
  }
};
...
exports.getTransferDetails = async (req, res) => {
  try {
    console.log("✅ POST /transfer-details called");
    const { transferId, ulbId } = req.body;
    ...
    const result = await service.getTransferDetailsService({ transferId, ulbId });
    res.json(result);
  } catch (error) {
    console.error("❌ Error in getTransferDetails:", error);
    res.status(500).json({ 
      success: false, 
      message: error.message || "Failed to fetch transfer details" 
    });
  }
};